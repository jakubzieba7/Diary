    ...
    public static class StudentConverter
    {
        ...
        public static List<Rating> ToRatingDao(this StudentWrapper model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(model.Math))
                model.Math.Split(',').ToList().ForEach(x => rating.Add(new Rating
                {
                    Rate=int.Parse(x),
                    StudentId=model.Id,
            ...
            if (!string.IsNullOrWhiteSpace(model.Physics))
                model.Physics.Split(',').ToList().ForEach(x => rating.Add(new Rating
                {
                    Rate = int.Parse(x),
                    StudentId = model.Id,
            ...
            if (!string.IsNullOrWhiteSpace(model.PolishLang))
                model.PolishLang.Split(',').ToList().ForEach(x => rating.Add(new Rating
                {
                    Rate = int.Parse(x),
                    StudentId = model.Id,
            ...
            if (!string.IsNullOrWhiteSpace(model.ForeignLang))
                model.ForeignLang.Split(',').ToList().ForEach(x => rating.Add(new Rating
                {
                    Rate = int.Parse(x),
                    StudentId = model.Id,
            ...
            if (!string.IsNullOrWhiteSpace(model.Technology))
                model.Technology.Split(',').ToList().ForEach(x => rating.Add(new Rating
                {
                    Rate = int.Parse(x),
                    StudentId = model.Id,
